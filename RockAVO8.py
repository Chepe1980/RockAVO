...
# Initialize default depth range
...
with st.sidebar:
    ...
    # Initialize depth range variables
    global_min_depth = DEFAULT_MIN_DEPTH
    global_max_depth = DEFAULT_MAX_DEPTH
    
    # If LAS file is uploaded, try to get depth range
    if uploaded_file:
        try:
            stringio = StringIO(uploaded_file.getvalue().decode("utf-8"))
            las = lasio.read(stringio)
            if 'DEPT' in las.curves:
                depth_curve = las['DEPT']
            else:
                # Try to find depth curve by common names
                for curve in las.curves:
                    if curve.mnemonic.lower() in ['depth', 'dept', 'depths']:
                        depth_curve = las[curve.mnemonic]
                        break
                else:
                    depth_curve = las[las.curves[0].mnemonic]
            
            global_min_depth = float(np.nanmin(depth_curve))
            global_max_depth = float(np.nanmax(depth_curve))
        except Exception as e:
            st.warning(f"Couldn't read depth curve: {str(e)}")
    
    st.subheader("Depth Range Selection")
    min_depth = st.slider("Minimum Depth (m)", 
                         min_value=float(global_min_depth), 
                         max_value=float(global_max_depth), 
                         value=float(global_min_depth),
                         step=1.0)
    
    max_depth = st.slider("Maximum Depth (m)", 
                         min_value=float(global_min_depth), 
                         max_value=float(global_max_depth), 
                         value=float(global_max_depth),
                         step=1.0)
...
if uploaded_file:
    try:
        ...
        # Create mask for selected depth range
        depth_mask = (depth_curve >= min_depth) & (depth_curve <= max_depth)
        z_well = depth_curve[depth_mask]
        ...
        def get_curve(name, alternatives, default=None):
            for alt in [name] + alternatives:
                if alt.lower() in curve_names:
                    ...
                    return curve_data[depth_mask]
            if default is not None:
                return np.linspace(default[0], default[1], len(z_well))
            return np.zeros(len(z_well))
        
        rho_well = get_curve('rhob', ['density', 'den'], [1.9, 2.3])
        vp_well = get_curve('vp', ['dtp', 'pwave'], [2400, 2800])
        vs_well = get_curve('vs', ['dts', 'swave'])
        if len(vs_well) == len(vp_well):
            vs_well = np.divide(vp_well, 1.7)
        else:
            vs_well = np.linspace(1400, 1700, len(z_well))
        gr_well = get_curve('gr', ['gamma', 'gammaray'])
        if len(gr_well) != len(z_well):
            gr_well = np.random.normal(50, 10, len(z_well))
...
# Prepare AVO curve data with bounds checking
top_layer = []
bottom_layer = []
...
for trace in range(len(syn_zoep)):
    try:
        ...
        idx1 = min(lyr1_indx[trace], syn_zoep.shape[1]-1)
        idx2 = min(lyr2_indx[trace], syn_zoep.shape[1]-1)
        
        top_layer.append(syn_zoep[trace, idx1])
        bottom_layer.append(syn_zoep[trace, idx2])
    except Exception as e:
        st.warning(f"Error extracting layer amplitudes for trace {trace}: {str(e)}")
        continue

if not top_layer or not bottom_layer:
    st.error("Could not extract layer amplitudes for AVO analysis")
    st.stop()

top_layer = np.array(top_layer)
bottom_layer = np.array(bottom_layer)

# Prepare time-domain properties with proper dimensions
vp_dig = np.full_like(t_samp, vp_data[0]/1000)  # Convert to km/s
vs_dig = np.full_like(t_samp, vs_data[0]/1000)
rho_dig = np.full_like(t_samp, rho_data[0])
...
with tab3:
    ...
    if len(syn_zoep) > 0:
        ...
        try:
            ...
            st.warning(f"Could not plot AVO curves: {str(e)}")
