...
# Custom Zoeppritz implementation with safety checks
...
for rc in rc_values:
    try:
        ...
        rc_series[100] = rc  # Place at 100th sample (adjust as needed)
        
        # Convolve with wavelet
        synthetic = np.convolve(rc_series, wlt_amp, mode='same')
        syn_seismic.append(synthetic)
    except Exception as e:
        st.warning(f"Error generating synthetic for angle {angle}°: {str(e)}")
        continue
